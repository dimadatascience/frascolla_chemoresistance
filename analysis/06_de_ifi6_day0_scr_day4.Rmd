...
params:
  ...
  sample_info_file: "./data/samples_info_sh1plus2_vs_scr.tsv"
  utils_file: "./src/__utils_rna_seq_functions.R"
  dds_file: "./data/deseq2.dds.RData" 
  ribosomal_genes_file: "./src/genes2filter_symbol.csv"
  hallmark_file: "./src/h.all.v2025.1.Hs.symbols.gmt"
  exp:
    value:
      condition: "shIFI6_Day0"
      description: "shIFI6_Day0"
  contr:
    value:
      condition: "shSCR_Day4"
      description: "shSCR_Day4"
---
...
# filtra campioni sperimentali
exp_samples <- subset(sample_info, condition == expA & !(grepl("^(c|tc|control)[0-9]*$", sample_type)))

# filtra campioni controllo
ctrl_samples <- subset(sample_info, condition == contrA & grepl("^(c|tc|control)[0-9]*$", sample_type))

# estrai nomi dei campioni (rownames)
exp_sample_names <- rownames(exp_samples)
ctrl_sample_names <- rownames(ctrl_samples)

# crea lista di confronto
deseq_group <- list(
  experimental = exp_sample_names,
  control = ctrl_sample_names,
  exp_conditions_text = unique(sample_info$condition_description[sample_info$condition == expA]),
  ctrl_conditions_text = unique(sample_info$condition_description[sample_info$condition == contrA])
)

if (is.null(deseq_group)) {
  stop("⚠️ No valid group for comparison ", expA, " vs ", contrA)
}

# use selected group for downstream analysis
selected_group <- deseq_group
```
...
if (na.omit(nrow(gseaA_list[["BP"]]@result)) > 0) {
  ...
  DT::datatable(
    gseaA_list[["BP"]]@result[, c("p.adjust", "enrichmentScore", "NES", "Description", "core_enrichment")],
    extensions = 'Buttons',
    options = list(
      dom = 'Blfrtip',
...
if (na.omit(nrow(gseaA_list[["CC"]]@result)) > 0) {
  ...
  DT::datatable(
    gseaA_list[["CC"]]@result[, c("p.adjust", "enrichmentScore", "NES", "Description", "core_enrichment")],
    extensions = 'Buttons',
    options = list(
      dom = 'Blfrtip',
      buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
...
if (na.omit(nrow(gseaA_list[["MF"]]@result)) > 0) {
  ...
  DT::datatable(
    gseaA_list[["MF"]]@result[, c("p.adjust", "enrichmentScore", "NES", "Description", "core_enrichment")],
    extensions = 'Buttons',
    options = list(
      dom = 'Blfrtip',
      buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
...
if (nrow(na.omit(gsea_result@result)) > 0) {
  ...
  DT::datatable(
    gsea_result@result[, c("p.adjust", "enrichmentScore", "NES", "Description", "core_enrichment")],
    extensions = 'Buttons',
    options = list(
      dom = 'Blfrtip',
      buttons = c('copy', 'csv', 'excel', 'pdf', 'print'),
